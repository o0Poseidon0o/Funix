...
namespace TMDTLaptop.Controllers
{
    public class AdminController : Controller
    {
        ...
        public ActionResult SuaSanPham(int id)
        {
            ...
            ViewBag.Hang = db.HangSanPham.ToList(); // Lấy danh sách hãng sản phẩm
            return View(sanPham);
        }
        ...
        public ActionResult SuaSanPham(SanPham model, HttpPostedFileBase HinhDaiDien, IEnumerable<HttpPostedFileBase> HinhKemTheo)
        {
            if (ModelState.IsValid)
            {
                ...
                sanPham.MaHang = model.MaHang;
                sanPham.NgayTao = DateTime.Now; // Nếu bạn muốn cập nhật ngày tạo
                ...
                if (HinhDaiDien != null && HinhDaiDien.ContentLength > 0)
                {
                    ...
                    // Tạo đường dẫn để lưu hình ảnh
                    var path = Path.Combine(Server.MapPath("~/assets/images/product/" + sanPham.MaSanPham + "/"));

                    // Tạo thư mục nếu chưa tồn tại
                    Directory.CreateDirectory(path);

                    // Lưu hình đại diện
                    HinhDaiDien.SaveAs(Path.Combine(path, sanPham.HinhAnh));
                }
                ...
                if (HinhKemTheo != null)
                {
                    ...
                    foreach (var hinh in HinhKemTheo)
                    {
                        ...
                        {
                            var fileName = Path.GetFileName(hinh.FileName);
                            var path = Path.Combine(Server.MapPath("~/assets/images/product/" + sanPham.MaSanPham + "/"), fileName);
        ...
        {
            var existingVoucher = db.Voucher.Find(voucher.MaVoucher); // Thay đổi Id thành thuộc tính ID thực tế của bạn
            ...
            {
                db.SaveChanges();
                return RedirectToAction("QuanLyVoucher"); // Hoặc nơi bạn muốn chuyển hướng sau khi cập nhật
            }
        ...
        public ActionResult ThemPhieuNhapKho(PhieuNhapKho phieuNhapKho, List<ChiTietPhieuNhapKho> ChiTietPhieuNhaps)
        {
            ...
            if (string.IsNullOrEmpty(phieuNhapKho.GhiChu))
            {
                ViewBag.ErrorMessage = "Vui lòng điền ghi chú nếu có.";
                ViewBag.SanPhams = db.SanPham.ToList();
