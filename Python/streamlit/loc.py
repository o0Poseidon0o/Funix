...
def filter_itemcode():
    ...
    try:
        # Lấy cột itemcode từ df1
        itemcode_file1 = df1['Itemcode']
        
        # Lấy cột itemcode từ df2 (cột thứ 2)
        itemcode_file2 = df2.iloc[:, 1]  # Cột thứ 2 trong file2
        
        # Lấy tất cả các cột từ df2 cho những itemcode không có trong df1
        result_df = df2[df2.iloc[:, 1].isin(itemcode_file1) == False]
    except KeyError as e:
        messagebox.showerror("Lỗi", f"Không tìm thấy cột Itemcode: {str(e)}")
        return
