...
const SettingRoleDepartment = () => {
  ...
  const [editedDepartment, setEditedDepartment] = useState({ id_departments: '', department_name: '', department_content: '' });
  ...
  const fetchDepartments = async () => {
    try {
      setLoading(true);
      const response = await axios.get('http://localhost:5000/api/departments/all-departments');
  ...
  const toggleEdit = async (department) => {
    if (editingId === department.id_departments) {
      if (!editedDepartment.department_name.trim() || !editedDepartment.department_content.trim()) {
        toast.error('Tên phòng ban và ghi chú không được để trống!');
        return;
      }
      try {
        await axios.put(`http://localhost:5000/api/departments/update/${department.id_departments}`, editedDepartment);
        setDepartments(prev => prev.map(d => d.id_departments === department.id_departments ? editedDepartment : d));
        setEditingId(null);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa không?")) {
      try {
        ...
        setDepartments(prev => prev.filter(department => department.id_departments !== id));
    ...
    <div className="w-full mt-8">
      ...
      <div className="bg-white overflow-auto">
        <table className="text-left w-full border-collapse">
          <thead>
            <tr>
              <th className="py-4 px-6 bg-gray-100 font-bold uppercase text-sm text-gray-600 border-b border-gray-300">Id Phòng Ban</th>
              <th className="py-4 px-6 bg-gray-100 font-bold uppercase text-sm text-gray-600 border-b border-gray-300">Tên Phòng Ban</th>
              <th className="py-4 px-6 bg-gray-100 font-bold uppercase text-sm text-gray-600 border-b border-gray-300">Ghi chú</th>
              ...
              <tr key={department.id_departments} className="hover:bg-gray-50">
                <td className="py-4 px-6 border-b border-gray-300">
                  {editingId === department.id_departments ? (
                    <input
                      type="text"
                      value={editedDepartment.id_departments}
                      onChange={(e) => handleEdit('id_departments', e.target.value)}
                ...
                <td className="py-4 px-6 border-b border-gray-300">
                  {editingId === department.id_departments ? (
                    <input
                      ...
                      onChange={(e) => handleEdit('department_name', e.target.value)}
                      className="border border-gray-300 rounded-md px-2 py-1"
                    />
                  ) : (
                ...
                <td className="py-4 px-6 border-b border-gray-300">
                  {editingId === department.id_departments ? (
                    <input
                      ...
                      onChange={(e) => handleEdit('department_content', e.target.value)}
                      className="border border-gray-300 rounded-md px-2 py-1"
                    />
                  ) : (
                ...
                <td className="py-4 px-6 border-b border-gray-300">
                  <button
                    ...
                    className={`bg-green-500 text-white px-4 py-2 rounded ${editingId === department.id_departments ? 'hidden' : ''}`}
                  >
                    Chỉnh sửa
                  </button>
                  <button
                    onClick={() => toggleEdit(department)}
                    className={`bg-yellow-500 text-white px-4 py-2 rounded ${editingId === department.id_departments ? '' : 'hidden'}`}
                  >
                    Lưu
                  </button>
                  <button
